...
```{r problem 1}
#days below freezing raw
plot(ts(Data1$DaysBelow32), col = "green", lwd = 2, main = "Days Below Freezing", ylab = "Temp (Degrees)") 
abline(h = mean(Data1$DaysBelow32), col = "tomato2", lwd = 2)
legend("topright", fill = "tomato2", expression("Average Days Below 32" *degree ~ F))

#days below freezing proportion
plot(ts(Data1$PropDays), col = "skyblue", lwd = 2, main = "Proportion of Days Below Freezing", ylab = "Temp (Degrees)") 
abline(h = mean(Data1$PropDays), col = "tomato2", lwd = 2)
legend("topright", fill = "tomato2", expression("Average Proportion of Days Below 32" *degree~F))

#should we be looking at year vs number of days below freezing?
...
The linear trend model for the proportion of measured days below freezing is given below. The t-test results are also given. _Holding the effect of other covariates constant, Based on the p-val..._ 

Testing for a linear trend in the proportion of days below freezing with a t-test yields a test statistic of -3.173 on 107 degrees of freedom, with an associated p-value of 0.00197, so we have strong evidence at the 5\% significance level that there is a linear trend in the proportion of days below freezing.
...
No idea why any of this is advantageous.
```{r problem 4}
library(mosaic)
#Permutation test: 
B<- 1000 
...
TstarA<-rep(12,A) 
for (a in (1:A)){ 
  TstarA[a]<-mblm(PropDays~shuffle(Year),data=Data1,repeated=FALSE)$coefficients[2]
  } 
```
