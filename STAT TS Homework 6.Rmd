...
```{r problem 6}
x=seq(1,109,by=1)
pvalue<-logical(length=250)
w<-as.data.frame(cbind(seq(1,109,by=1),rep(12,109)))
colnames(w)<-c("index","noise")
for(i in 1:250) {
w[,2]<-rnorm(109,mean=0,sd=sd(model1s$residuals))
slope.nt<-summary(lm(index~noise,data=w))$coefficients[2,4]
pvalue[i]<-slope.nt<.05
}
sum(pvalue==TRUE)/250

coef<-rep(12,250)
inthere<-logical(length=250)
for (j in (1:250)){ 
  shuf<-shuffle(w$index)
  model<-mblm(index~noise,data=w,repeated=FALSE)
  coef[j]<-model$coefficients[2]
  CI<-confint(model)[2,]
  inthere[j]<-(0>CI[1])&(0<CI[2])
}
FTR<-sum(inthere==TRUE)
R<-sum(inthere==FALSE)
R/250
```
...
The Zhang method is given below. We include the confidence intervals below. Both methods give intervals that _do not_ contain $0$; however, the interval coming from the Zyp package is slightly wider than the interval fro the MBLM method. In either case, it appears that the proportion of days below freezing is decreasing over time - both methods tell this story. 
